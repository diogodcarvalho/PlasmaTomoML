...
print 'ssim', 'psnr', 'nrmse', 'total_power_error'
...
for pulse,t,g_mi,g_i in zip(pulse_valid,t_valid,g_m,g_valid):
    ...
    total_power_error = (np.sum(g_i) - np.sum(g_mi))/np.sum(g_i)
    print '%.4f %.2f %.2f %.2f' % (ssim[0],psnr[0],nrmse[0],total_power_error)

    SSIM.append(ssim)
    PSNR.append(psnr)
    NRMSE.append(nrmse)
    POWER.append(total_power_error)
